...
ts1=pd.read_csv(site[0]+'botttemp_mc_obs.csv',index_col=0,names=['yy','mm','dd','count','mean','median','min','max','std'])
dfmean1=df1.mean()
...
for k in range(len(site)):
    if k!=0:
        ...
        ts=pd.read_csv(site[k]+'botttemp_mc_obs.csv',index_col=0,names=['yy','mm','dd','count','mean','median','min','max','std'])
        dfmean=df.mean()
