...
for i in range(len(df)):
    if df['mean'][i]>=np.float64(0):
        ...
        if 50*df['mean'][i]>=np.float64(70) and df.index[i]<>'TA15':
            plt.annotate(df.index[i],xy=(x[i],y[i]),xytext=(x[i]+0.51,y[i]+0.35),arrowprops=dict(facecolor='red', shrink=0.2))
        if df.index[i]=='TA15':
                 plt.annotate(df.index[i],xy=(x[i],y[i]),xytext=(x[i]-0.31,y[i]+0.45),arrowprops=dict(shrink=0.25,facecolor='red'))
    else:
        print x[i],y[i]
        m.scatter(x[i],y[i],50*(-df['mean'][i]),marker='o',color='blue')
        print 50*df['mean'][i]
        if 50*(-df['mean'][i])>=np.float64(70):
            plt.annotate(df.index[i],xy=(x[i],y[i]),xytext=(x[i]+0.51,y[i]+0.35),arrowprops=dict(facecolor='blue', shrink=0.2))
