...
#            fv=np.load(FNV)
            ...
            u0[i],v0[i],kedh[i]=VelInterp_lonlat(lonp,latp,Grid,fu,fv)   
# note that kedh is in fact kv, vertex corresponding to a polygon of the dual mesh 
        ...
        if np.isnan(u0).all():
            ...
            print FN2
            np.savez(FN2,tdh=tdh,kedh=kedh,londh=londh,latdh=latdh,udh=udh,vdh=vdh,umoh=u0,vmoh=v0)
            #data(name,time period,neiberpoints,lon interpolation,lat interpolation,ureal,vreal,umod,vmod)
