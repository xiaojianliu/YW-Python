...
def basemap_usgs(lat,lon,bathy):
    ...
    #plt.contourf(X,Y,basemap_topo.topo[min_index_lat:max_index_lat,min_index_lon:mabasemapx_index_lon],[-90,-60],colors='black')
...
def basemap_detail(lat,lon,bathy,draw_parallels,*parallels_interval):
    ...
    if draw_parallels==True:
      ...
      if len(parallels_interval)<1:
        parallels_interval=1
        #draw parallels     
        m.drawparallels(np.arange(int(min(lat)),int(max(lat)),fbasemaploat(parallels_interval)),labels=[1,0,0,0],fmt=lat2str,dashes=[2,2])
        #draw meridians
